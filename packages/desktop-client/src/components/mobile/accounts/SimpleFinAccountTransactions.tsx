...
export function SimpleFinAccountTransactions() {
...
function TransactionListWithPreviews() {
  ...
  const baseTransactionsQuery = useCallback(
    () => queries.transactions('simplefin').options({ splits: 'all' }).select('*'),
    [],
  );
  ...
  const simpleFinAccountIds = useMemo(
    () =>
      accounts
        .filter(
          account =>
  ...
  const simpleFinAccountsFilter = useCallback(
    (schedule: ScheduleEntity) => simpleFinAccountIds.includes(schedule._account),
    [simpleFinAccountIds],
  );

  const { previewTransactions, isLoading: isPreviewTransactionsLoading } =
    usePreviewTransactions({
      filter: simpleFinAccountsFilter,
    });
  ...
  return (
    <TransactionListWithBalances
      ...
      searchPlaceholder={t('Search SimpleFIN Accounts')}
      onSearch={onSearch}
